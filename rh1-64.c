...
void help()
{
 char *message = "Data Dependent Rotation Hash, 64-bit\n\n"
 "options\n"
...
void transform(uint64_t *s, unsigned *reo, unsigned swn, unsigned r)
{
...
int check_o(struct options *opts)
{
 ...
 if((opts->l % 8) || (opts->l > (opts->swn * 32)))
 {
...
/* load data block */
bool load_block(FILE *fp, uint64_t *s, unsigned b)
{
 int i, c;
 ...
 for (i = 0; i < b; i++)
 {
  if((c = getc(fp)) == EOF)
  {
   s[i / 8] ^= ((uint64_t)0x80) << ((7 - (i & 7)) * 8);
   return false;
  }

  s[i / 8] ^= ((uint64_t)c) << ((7 - (i & 7)) * 8);
 }
...
void extract_h(uint64_t *s, unsigned l, uint8_t *h)
{
 int i, end;
...
void hash_files(struct options *opts)
{
 ...
 if((hs.s = malloc(opts->swn * 8)) == NULL) error("allocate state");
...
unsigned check_files(FILE *cf, struct options *opts)
{
 ...
 while(true)
 {
  ...
  if((hs.s = realloc(hs.s, opts->swn * 8)) == NULL) error("allocate state");
